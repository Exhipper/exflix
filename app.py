...
def api_add_account():
    """Admin endpoint to add + validate a new cookie."""
    ...
    plan = data.get("plan", "Premium").strip() or "Premium"
    country = data.get("country", "US").strip() or "US"
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "message": "Account added and validated successfully!",
            "account_id": new_id,
            "token_expires": datetime.fromtimestamp(expires).strftime("%Y-%m-%d %H:%M:%S") if isinstance(expires, (int, float)) else str(expires)
        })

    except Exception as e:
        return jsonify({"success": False, "error": f"Validation or insert failed: {str(e)}"}), 400
